    ...
    try:
        ...
        if not os.path.exists(".git"):
            if upstream_repo:
                ...
                if not success:
                     # Fallback to main if master/configured failed
                ...
                updated_during_init = True
            else:
                 await msg.edit("No .git folder found and no UPSTREAM_REPO configured.")
                 return

        # If we didn't just update via init, check for standard updates
        if not updated_during_init:
            # Fetch updates
            success, output = run_command("git fetch origin")
            if not success:
